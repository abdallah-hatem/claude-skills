...
| **build-software** | `/build-software` only — a new app from idea to production, or a feature added to an existing app. Business doc, edge-case tests, a design approval, git flow with PRs into `dev` and `production`, preview and production deploys on Vercel, and test logins kept in a gitignored file |
